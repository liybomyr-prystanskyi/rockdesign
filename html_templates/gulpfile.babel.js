  ...
  return gulp.src(['src/assets/templates/head.mustache', 'src/assets/templates/foot.mustache'])
    ...
    .pipe(cheerio(function($, file) {
      ...
      $('link').each(function() {
        ...
        $(this).attr('href', '<?php echo get_stylesheet_directory_uri(); ?>/html_templates/build/' + linkHref);
      });
      // Src
      $('script').each(function() {
        if($(this).attr('src')) {
          let scriptSrc = $(this).attr('src');
          $(this).attr('src', '<?php echo get_stylesheet_directory_uri(); ?>/html_templates/build/' + scriptSrc);
        }
      });      
    }))
    .pipe(inject.replace('&lt;', '<'))
  ...
  return gulp.src(sysConfig.paths.src.templates.molecules+'*.mustache')
    ...
    .pipe(cheerio(function($, file) {
      $('img').each(function() {
        ...
        $(this).attr('src', '<?php echo get_stylesheet_directory_uri(); ?>/html_templates/build/' + imgSrc);
      });
    }))
    .pipe(inject.replace('&lt;', '<'))
    .pipe(inject.replace('&gt;', '>'))
